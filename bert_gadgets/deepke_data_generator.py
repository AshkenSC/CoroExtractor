# 根据ngram句子数据创建deepke的训练数据集
# 先从ngram里找到存在关系的句子，然后不转为三元组，而是保留原句子并按照数据集要求的格式输出结果
# sentence, relation, head, head_index, tail, tail_index
#---------------------------------------------------------------------
...
DATA_SOURCE = r'f:\Projects\corona\hudong_data\ngram\ngrams_addline\detail\3_detail.txt'
OUTPUT = r'F:\Projects\deepke-master\data\origin\data_hudong.txt'
...
def get_relation_name(triple, category):
    if category == 0:
        return '可医治'
    elif category == 1:
        return '推荐药物'
...
# 获取head和tail在句子中的起始位置
# 返回两个值：head和tail的起始字符index
def get_entity_index(triple, category):
    if category == 5:
        return str(len(triple[0]) + len(triple[2])), str(0)
    else:
        return str(0), str(len(triple[0]) + len(triple[2]))
...
for line in data_source:
    if 'calculate' in line:
        ...
        if is_valid(new_triple, current_category):
            # 为了构造训练集，不需要替换rel关键词
            # new_triple = get_relation_name(new_triple, current_category)
            # 需要记录当前句子包含哪种关系
...
for sublist in valid_triples:
    for triple in sublist:
        if tuple(triple) not in dedup_triple_list:
            ...
            # 拼接句子
            sentence = triple[0] + triple[2].strip('\n') + triple[1]
            # 定位实体位置
            head_index, tail_index = get_entity_index(triple, rel_category_num)
            # 输出单条数据
            output.write(sentence +',' + get_relation_name(triple, rel_category_num) + ',' + triple[0] + ',' + head_index + ',' + triple[1] + ',' + tail_index + '\n')
    rel_category_num += 1
output.close()